...
//*/
    ...
    private static void printWinner(TeamColor winner) {
        if (winner == null) {
            console.println("Ended in stalemate");
        }
        else {
            console.println("Winner: " + winner.toString());
        }
    }
    ...
    private static void printGame(GameBasicInfo game, Integer index) throws ServerFacadeException {
        ...
        String activePlayer;
        if (game.whiteActive == null) {
            activePlayer = null;
        }
        else if (game.whiteActive) {
            activePlayer = "WHITE";
        }
        else {
            activePlayer = "BLACK";
 ...
 /*       TeamColor currColor = null;
    ...
    private static void checkGameConditions(boolean includeUsername) {
        GameObj status = currGame.getStatus();
        boolean finished = status.getGameOver();
        TeamColor winner = status.getWinner();
        TeamColor inCheck = status.getCheckedTeam();
        TeamColor checkmated = status.getCheckmatedTeam();
        TeamColor activePlayer = status.getTeamTurn();

        String winnerStr = "null";
        String inCheckStr = "null";
        String checkmatedStr = "null";
        String activePlayerStr = "null";
        if (includeUsername) {
            if (winner != null) {winnerStr = colorToPlayer(winner);}
            if (inCheck != null) {inCheckStr = colorToPlayer(inCheck);}
            if (checkmated != null) {checkmatedStr = colorToPlayer(checkmated);}
            if (activePlayer != null) {activePlayerStr = colorToPlayer(activePlayer);}
        }
        else {
            if (winner != null) {winnerStr = winner.toString();}
            if (inCheck != null) {inCheckStr = inCheck.toString();}
            if (checkmated != null) {checkmatedStr = checkmated.toString();}
            if (activePlayer != null) {activePlayerStr = activePlayer.toString();}
        }

        if (checkmated != null) {
            console.print('\n' + checkmatedStr + " is in checkmate.");
        }
        else if (inCheck != null) {
            console.println('\n' + inCheckStr + " is in check.");
        }
        if (finished && winner == null) {
            console.print('\n' + activePlayerStr + " has no legal moves.");
        }
        if (finished) {
            if (winner != null) {
                console.println('\n' + winnerStr + " wins!");
                ...
                if (!skipValidityCheck) {
                    try {
                        ...
                        if (validMove == null) {
                            ...
                            console.println("Proper move format example: a2 a4");
                            validMove = false;
                        }
                    }
    ...
    private static boolean checkForNewMoves() throws Exception {
        ...
        try {
            for (String moveStr : newMoves) {
                ...
                try {
                    currGame.getStatus().makeMove(new MoveObj(moveStr));
                } catch (InvalidMoveException x) {
                    throw ExceptionConverter.convert(x);
                }

                describeMove(moveStr, true);
            }
        }
    ...
    private static Boolean parseMove(String moveStr) throws IOException, InvalidMoveException {
        ...
        if (!Position.isValidPos(pos1) || !Position.isValidPos(pos2)) {
            return null;
        }
            ...
            if (pendingBytes > 0) {
                ...
                switch (userInput) {
                    ...
                    case "resign":
                        ...
                        if (currGame.getStatus().getGameOver()) {
                            console.println("Game is already over");
                            break;
                        }
                        if (currGame.getStatus().getTeamTurn() == myColor) {
                            handleResignation();
    ...
    private static void handleMoveHighlight() throws Exception {
        ...
        while (!done) {
            skipValidityCheck = false;
            if (pendingBytes > 0) {
                console.print("Enter a position (\"done\" to finish): ");
            }
            pendingBytes = reader.available();
            buffer = new byte[MAX_USERNAME_SIZE];
            ...
            if (pendingBytes > 0) {
                ...
                if (checkForCancel(positionStr, true) || positionStr.equalsIgnoreCase("done")) {
                    break;
                }
                ...
                if (!skipValidityCheck) {
                    ...
                    if (chosenPiece != null) {
                        printHighlightedBoard(chosenPiece);
                    }
                }
            }
        }
    ...
    /**
     * Be sure to call this function only AFTER the move is made!
     */
    private static void describeMove(String moveStr, boolean includeUsername) throws InvalidPositionException {
        boolean isResignation = false;
        MoveObj theMove = new MoveObj(moveStr);
        if (moveStr.equals("R")) {
            isResignation = true;
        }
        TeamColor activePlayer;
        activePlayer = GameObj.otherTeam(currGame.getStatus().getTeamTurn()); // Since we are calling this
        // AFTER making the move, we have to use the previous active player
        if (isResignation) {
            activePlayer = GameObj.otherTeam(activePlayer);
        } // If, however, the move was a resignation, the game never had a chance to switch turns
        StringBuilder moveDescrip;
        String playerString;
        if (includeUsername) {playerString = colorToPlayer(activePlayer);}
        else {playerString = activePlayer.toString();}

        if (!isResignation) {
            console.println('\n' + playerString + " made a move:");
    ...
    private static void constructGame(int gameID) throws IOException, InvalidPositionException,
            ServerFacadeException, InvalidMoveException {
        serverF.requestBoard(authToken, gameID);
        GameObj gameStatus = new GameObj();
        GameBasicInfo thisGameInfo = null;
        var oldMoves = new ArrayList<String>();

        while (!serverF.prevMoveMsgAvailable()) {}
        oldMoves = serverF.getOldMoves();
        ...
        currGame = new models.Game(gameID, thisGameInfo.gameName, gameStatus, null,
                thisGameInfo.whiteUsername, thisGameInfo.blackUsername);

        printBoard();

        for (int i = 0; i < oldMoves.size(); i++) {
            gameStatus.makeMove(new MoveObj(oldMoves.get(i)));
            describeMove(oldMoves.get(i), false);
    ...
    static private boolean joinGame(int gameID, TeamColor color)
        ...
        if (result.getResponseCode() == 200) {
            try {
                serverF.webSocket(authToken, gameID, color, myUsername);
            }
